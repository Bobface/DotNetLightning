...
let tests =
    ...
    testList "BOLT-11 tests" [
        ...
        testCase "Same, but adding invalid unknown feature 100" <| fun _ ->
            ...
            Expect.isError(pr.ToFSharpCoreResult()) ""
    ]
...
let unitTest =
    ...
    testList "PaymentRequest Unit tests" [
        testCase "Can create PaymentRequest correctly" <| fun _ ->
            ...
            Expect.isOk (r2.ToFSharpCoreResult()) ""
            ...
            Expect.isError (r3.ToFSharpCoreResult()) "Field contains both description and description hash! this must be invalid"
