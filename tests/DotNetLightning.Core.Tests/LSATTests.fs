...
let lsatTests =
    testList "LSAT tests" [
        testCase "service decode tests" <| fun _ ->
            ...
            Expect.isOk (r.ToFSharpCoreResult()) "can parse single service"
            Expect.equal 1 (r |> Result.deref).Count ""
            Expect.equal "a" (r |> Result.deref).[0].Name ""
            let r = Service.ParseMany("a:0,b:1,c:0")
            Expect.isOk (r.ToFSharpCoreResult()) "can parse multiple service"
            ...
            Expect.isError (r.ToFSharpCoreResult()) "can not parse empty service"
            let r = Service.ParseMany ":a"
            Expect.isError (r.ToFSharpCoreResult()) "can not parse service missing name"
            let r = Service.ParseMany "a"
            Expect.isError (r.ToFSharpCoreResult()) "can not parse service missing tier"
            let r = Service.ParseMany "a:"
            Expect.isError (r.ToFSharpCoreResult()) "can not parse service with empty tier"
            let r = Service.ParseMany ",,"
            Expect.isError (r.ToFSharpCoreResult()) "can not parse empty services"
            ()
