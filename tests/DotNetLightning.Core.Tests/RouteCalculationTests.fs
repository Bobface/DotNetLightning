...
let tests = testList "Route Calculation" [
    ...
    testCase "Calculate simple route (add and remove edges)" <| fun _ ->
        ...
        Expect.isError (route2.ToFSharpCoreResult()) ""
    ...
    testCase "find a route using channels with htlcMinimumMsat close to the payment amount" <| fun _ ->
        ...
        let route =
            Routing.findRoute graph f i DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))
        Expect.isError (route.ToFSharpCoreResult()) ""
    ...
    testCase "route not found (source OR target node not connected)" <| fun _ ->
        ...
        Expect.isError ((Routing.findRoute g a d DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))).ToFSharpCoreResult()) ""
        Expect.isError ((Routing.findRoute g b e DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))).ToFSharpCoreResult()) ""
    ...
    testCase "route not found (amount too high OR too low)" <| fun _ ->
        ...
        Expect.isError ((Routing.findRoute gHigh a d highAmount 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))).ToFSharpCoreResult()) ""
        Expect.isError ((Routing.findRoute gLow a d lowAmount 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))).ToFSharpCoreResult()) ""
    ...
    testCase "blacklist routes" <| fun _ ->
        ...
        Expect.isError (route1.ToFSharpCoreResult()) ""
    ...
    testCase "route to a destination that is not in the graph (with assisted routes)" <| fun _ ->
        ...
        let route =
            Routing.findRoute(g) a e DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight(400000u))
        Expect.isError(route.ToFSharpCoreResult()) "there should be no e node in the graph"
    ...
    testCase "limit routes to 20 hops" <| fun () ->
        ...
        Expect.isError(r21.ToFSharpCoreResult()) ""
    ...
    testCase "select a random route below the requested fee" <| fun _ ->
        ...
        for _ in 0..10 do
            ...
            Expect.isOk (r.ToFSharpCoreResult()) ""
